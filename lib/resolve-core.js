...
/**
 * Nominatim's viewbox, "left,top,right,bottom", checked before it is passed on:
 * four finite numbers inside the globe, or nothing. A viewbox only makes a
 * search prefer an area, never confines it, so a bad one is dropped rather than
 * refused -- the search still works, just without the hint.
 */
export function cleanViewbox(raw) {
  if (raw == null || raw === '') return null;
  const parts = String(raw).split(',').map((v) => Number(v.trim()));
  if (parts.length !== 4 || !parts.every(Number.isFinite)) return null;
  const [left, top, right, bottom] = parts;
  if ([left, right].some((v) => v < -180 || v > 180)) return null;
  if ([top, bottom].some((v) => v < -90 || v > 90)) return null;
  return parts.map((v) => v.toFixed(4)).join(',');
}

export async function geocode(query, env, limit = 1, viewbox = null) {
  ...
  try {
    ...
    if (query) {
      const results = await geocode(query, env, Number(params.get('limit')) || 1,
        cleanViewbox(params.get('viewbox')));

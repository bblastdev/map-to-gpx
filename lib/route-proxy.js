...
async function bump(bucket, ttlSeconds, env) {
  ...
  if (!url || !key) return { count: bumpMemory(bucket, ttlSeconds), backend: 'memory' };
  try {
    const res = await fetch(url.replace(/\/$/, '') + '/rest/v1/rpc/bump_counter', {
      method: 'POST',
    ...
    if (!Number.isFinite(count)) throw new Error('bad rpc payload');
    return { count, backend: 'supabase' };
  } catch (_) {
    /* Never fail a route because the counter store is down — fall back to the
       in-memory limiter, which is stricter per instance, not looser. */
    return { count: bumpMemory(bucket, ttlSeconds), backend: 'memory-fallback' };
  }
}
